...
## output, and enforce safety constraints including dangerous-
## command detection, command-pattern regex validation, and
## interactive manual-confirm gating.  Commands are run through the
## configured shell via startProcess with the bundled bin/ directory
## prepended to PATH so that shipped tools (rg, fd, sg, pmc,
...
## Checks whether a command contains any known dangerous commands.
## Returns the name of the first dangerous command found, or none
## if the command appears safe.  The check splits the command into
## tokens and compares each against the DANGEROUS_COMMANDS list.
##
## :param command: The command string to check.
## :returns: The dangerous command name, or none if safe.
##
## .. code-block:: nim
##   runnableExamples:
##     import std/options
##     assert checkDangerousCommand("ls -la").isNone
##     assert checkDangerousCommand("rm -rf /").isSome
proc checkDangerousCommand*(
  command: string
): Option[string] =
  # Normalise by replacing common shell operators with spaces
  # so that tokens like "&&rm" or ";rm" get split.
  var normalised = command
  for ch in ['|', ';', '&', '(', ')', '`', '$']:
    normalised = normalised.replace(ch, ' ')
  let tokens = normalised.splitWhitespace()
  for token in tokens:
    let t = token.strip()
    if t.len == 0:
      continue
    # Extract the base command name (last path component).
    let base =
      when defined(windows):
        t.split({'/', '\\'})
      else:
    ...
    for danger in DANGEROUS_COMMANDS:
      if danger in CASE_SENSITIVE_DANGER:
        # Case-sensitive match for short ambiguous names.
        if name == danger:
          return some(danger)
      else:
        # Case-insensitive match for most commands.
        if toLowerAscii(name) == toLowerAscii(danger):
          return some(danger)
  return none(string)

## Checks whether a user-provided command-pattern regex would
## match any of the known dangerous commands.  Returns a warning
## message listing unblocked dangerous commands, or an empty
## string when all are blocked (i.e. the pattern does not match
## them).
##
## :param pattern: The user's command-pattern regex.
## :returns: Warning text, or empty string if safe.
##
## .. code-block:: nim
##   runnableExamples:
...
proc checkPatternSafety*(
  ...
  for danger in DANGEROUS_COMMANDS:
    try:
      if danger.contains(re2(pattern)):
        allowed.add(danger)
    except CatchableError:
      discard
  if allowed.len > 0:
    result = "warning: command-pattern allows " &
