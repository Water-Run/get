## Adversarial tests for the mandatory read-only command policy.
##
...
suite "mandatory read-only command policy":
  ...
  test "executables whose long parameter tables were removed are denied":
    for command in [
      "sed -n '1,20p' README.md", "cat README.md | sed 's/a/b/'",
      "nvidia-smi", "nvidia-smi -L", "rocm-smi --showuse", "rocminfo",
      "docker ps", "podman images", "kubectl get pods -A",
      "apt list --installed", "dnf info bash", "rpm -qa", "brew outdated",
      "winget upgrade", "pip list", "npm view react version", "snap list",
      "ufw status", "firewall-cmd --state", "nft -j list ruleset",
      "iptables -L -n", "cargo search serde", "nimble list -i"
    ]:
      checkpoint(command)
      check not checkReadOnlyCommand(command, "bash").allowed
    for command in [
      "Get-Process | Where-Object Name -eq bash", "Get-NetFirewallProfile",
      "Get-AppxPackage"
    ]:
      checkpoint(command)
      check not checkReadOnlyCommand(command, "powershell").allowed
