...
`get` 把自然语言问题转换为只读的本地查询。v4 增加环境、文件、搜索和直接进程工具，支持普通 Git status/diff，并在具备隔离能力的 Linux 上执行短脚本与组合计算。它仍然只做查询；模型名称作为不透明的服务标识传递。v4 正在验证，当前结果见 [开发记录](DEVELOPMENT-v4.0.0.md)。
