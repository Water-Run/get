...
- **后续工作区已修复：`NO_PROXY` 的端口限定。** 本轮初始审查时，解析会移除条目中的单个端口，`host:port` 因而可能扩大为整台主机绕过代理。后续基于 `331c9df` 的修复和独立验证见文末，尚不作为已发布载荷的验证结果。
...
**2026-09-07 后续修复：代理绕过的端口和 IP 地址边界。**

基于 `331c9df`，修复 [llm.nim](src/llm.nim) 中 `NO_PROXY` 的匹配逻辑：`provider.test:443` 只绕过目标端口 443；URL 未指定端口时按 HTTP 80、HTTPS 443 判断。未限定端口的主机条目仍适用于所有端口，域名及其子域名的既有匹配行为保留。IP 字面量按地址精确比较；IPv6 端口使用 `[::1]:8080`，未加括号的 IPv6 整体作为地址处理。空端口、非数字端口、越界端口和非法括号形式不会扩大绕过范围。

带端口主机条目的语义参考 [Python urllib.request 官方说明](https://docs.python.org/3/library/urllib.request.html#urllib.request.ProxyHandler)。实现直接解析 URL、端口与 IP 字面量，不为匹配执行 DNS 查询。中英文 README 和 man 页面已补充格式与边界说明。

先加入回归、再修复实现：新增的 6 组路由测试均能在旧实现上暴露错误，修复后全部通过。针对性回归覆盖显式及默认端口、域名边界、IPv4、IPv6、非法条目和小写环境变量；本地代理集成测试进一步检查匹配端口时零代理请求、不匹配端口及非法端口时实际经过代理。

| 本次验证 | 结果 |
|---|---|
| LLM 定向单元测试 | 17/17，通过；包含新增的 6 组路由测试；[日志](.ci/proxy-20260907/final-llm.log) |
| Linux Bash CLI 完整回归 | 38/38，通过；使用本地模拟服务和隔离配置；[日志](.ci/proxy-20260907/cli.log) |
| Windows amd64 / macOS arm64 | 定向测试及传输模块的 `nim check` 均通过；未运行原生目标二进制 |
| 构建与文档 | Nim 2.2.10 release 构建、Python 语法、man 渲染和改动空白检查通过 |

由于工作区同时存在独立进行中的配置/日志改动，本次 CLI 与跨平台静态验证使用 `331c9df` 加代理实现、定向单元测试和 CLI 测试三个文件的源码快照，没有覆盖其他工作区改动。[源码清单](.ci/proxy-20260907/source-manifest.json)和[验证摘要](.ci/proxy-20260907/validation-summary.json)绑定该范围及二进制摘要。编译使用 `--parallelBuild:1`，编译器报告峰值内存约 271 MiB；验证任务顺序运行。本次未操作 GNOME/AppIndicator 会话、调用真实模型服务、替换安装副本或发布版本。
