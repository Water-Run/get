...
def err(
    msg: str) -> None: print(f"{C.BOLD}{C.RED}error:{C.RESET} {msg}", file=sys.stderr)
...
def ask(prompt: str, default: str = "y") -> bool:
    ...
    while True:
        try:
            a = input(
                f"{C.BOLD}?{C.RESET} {prompt} {C.DIM}{suffix}{C.RESET} ").strip().lower()
        except EOFError:
            a = ""
        if not a:
            a = default.lower()
...
def _finish_install(paths: dict) -> None:
    ...
    print(
        f"     {C.BOLD}get version{C.RESET}    {C.DIM}# verify installation{C.RESET}")
    print(
        f"     {C.BOLD}get help{C.RESET}       {C.DIM}# show command help{C.RESET}")
    if IS_LINUX:
        print(
            f"     {C.BOLD}man get{C.RESET}        {C.DIM}# read the manual{C.RESET}")
    print()
